...
import {
  createAytActivity,
  createAytActivitySlug,
  deleteAytActivity,
  deleteAytActivityPhoto,
  getAytActivityPhotoUrl,
  aytActivityCategories,
  listAllAytActivities,
  updateAytActivity,
  uploadAytActivityPhoto,
  type AytActivityCategory,
  type AytActivityRecord,
  type AytActivityStatus,
...
const emptyForm: ActivityFormState = {
  ...
  location: "AYT Agro Farm, Dramaga, Kabupaten Bogor",
  category: "Peternakan",
  excerpt: "",
  content: "",
  image_label: "",
...
export default function AdminAytActivitiesPanel() {
  ...
  const [statusFilter, setStatusFilter] =
    useState<"all" | AytActivityStatus>("all");
  ...
  const loadActivities = useCallback(async () => {
    ...
    try {
      const rows = await listAllAytActivities();
      setActivities(rows);
    } catch (error) {
  ...
  const handlePhotoUpload = async (
    ...
    try {
      for (const file of files) {
        const uploaded = await uploadAytActivityPhoto(
          file,
          form.status,
        );
  ...
  const saveActivity = async (
    ...
    try {
      ...
      if (selectedActivity) {
        const updated = await updateAytActivity(
          selectedActivity.$id,
          selectedActivity,
          payload,
        );
        ...
        const created = await createAytActivity(payload);
  ...
  const removeActivity = async (
    ...
    try {
      await deleteAytActivity(activity);
  ...
  return (
    <div className="admin-activities-panel">
      <div className="admin-panel-heading">
        <div>
          <h2>Dokumentasi Kegiatan AYT Agro Farm</h2>
          <p>
      ...
      <div className="admin-toolbar admin-activity-toolbar">
        <input
          ...
          aria-label="Cari kegiatan AYT Agro Farm"
        />
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      setField(
                        "category",
                        event.target
                          .value as AytActivityCategory,
                      )
                    }
                  >
                    {aytActivityCategories.map(
                      (category) => (
                        <option
                          key={category}
                          value={category}
                      ...
                      >
                        <div>
                          <Image
                            ...
                            alt="Dokumentasi kegiatan AYT Agro Farm"
                            fill
                            unoptimized
                            sizes="180px"
                          />
